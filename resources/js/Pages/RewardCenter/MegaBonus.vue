<template>
    <DashboardLayout title="Mega Bonus">
        <!-- Breadcrumb -->
        <div class="bg-gradient-to-r from-orange-500/20 to-red-500/20 backdrop-blur-sm text-white px-4 lg:px-8 py-3 flex items-center space-x-2 text-sm border-b border-orange-500/20">
            <Crown :size="16" class="text-orange-400" />
            <span>Reward Center</span>
            <span class="text-orange-400">›</span>
            <span class="text-orange-300">Mega Bonus</span>
        </div>
        ...
        <div class="p-4 lg:p-8">
            <div class="max-w-4xl mx-auto">
                <!-- Bonus Card -->
                <div class="bg-gradient-to-br from-orange-600/40 to-red-600/40 backdrop-blur-xl border border-orange-500/30 rounded-2xl p-8 shadow-2xl shadow-orange-500/20 mb-6">
                    <div class="text-center">
                        <Crown :size="64" class="mx-auto text-orange-400 mb-4" />
                        <h1 class="text-4xl font-bold text-white mb-2">{{ bonusInfo.name }}</h1>
                        <p class="text-orange-200 text-lg mb-6">{{ bonusInfo.description }}</p>
                        
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-2xl mx-auto">
                            <!-- Bonus Amount -->
                            <div class="bg-black/30 rounded-xl p-6 border border-orange-500/20">
                                <p class="text-orange-300 text-sm mb-2">You Get</p>
                                <p class="text-3xl font-bold text-white">KES {{ bonusInfo.bonus_amount.toLocaleString() }}</p>
                            </div>
                            
                            <!-- Claim Cost -->
                            <div class="bg-black/30 rounded-xl p-6 border border-orange-500/20">
                                <p class="text-orange-300 text-sm mb-2">Cost</p>
                                <p class="text-3xl font-bold text-white">KES {{ bonusInfo.claim_cost.toLocaleString() }}</p>
                            </div>
                        </div>
                        
                        <!-- Net Profit -->
                        <div class="mt-6 bg-gradient-to-r from-red-700/40 to-orange-700/40 rounded-xl p-4 max-w-md mx-auto">
                            ...
                            <p class="text-yellow-200 text-xs mt-1">That's {{ ((bonusInfo.bonus_amount - bonusInfo.claim_cost) / bonusInfo.claim_cost * 100).toFixed(0) }}% return on investment!</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-xl border border-orange-500/30 rounded-2xl p-6 shadow-2xl shadow-orange-500/20">
                    <div class="flex flex-col lg:flex-row items-center justify-between gap-6">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-2xl font-bold text-orange-400">KES {{ userBalance.toLocaleString() }}</p>
                        ...
                        <div class="flex flex-col items-center space-y-3">
                            <button
                                ...
                                class="bg-gradient-to-r from-orange-600 to-red-600 hover:from-orange-500 hover:to-red-500 disabled:from-slate-600 disabled:to-slate-600 disabled:cursor-not-allowed text-white font-bold py-4 px-8 rounded-xl transition-all duration-200 shadow-lg shadow-orange-500/50 flex items-center space-x-2 text-lg min-w-[200px]"
                            >
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
                    <!-- How It Works -->
                    <div class="bg-gradient-to-br from-amber-600/40 to-yellow-600/40 backdrop-blur-xl border border-amber-500/30 rounded-2xl p-6 shadow-2xl shadow-amber-500/20">
                        ...
                        <ul class="space-y-3 text-sm">
                            <li class="flex items-start space-x-2">
                                ...
                                <span class="text-amber-100">KES {{ bonusInfo.claim_cost.toLocaleString() }} is deducted from your deposit balance</span>
                            </li>
                            <li class="flex items-start space-x-2">
                                <CheckCircle :size="16" class="text-green-400 mt-0.5 flex-shrink-0" />
                                <span class="text-amber-100">KES {{ bonusInfo.bonus_amount.toLocaleString() }} is added to your balance</span>
                            </li>
                            <li class="flex items-start space-x-2">
                                <CheckCircle :size="16" class="text-green-400 mt-0.5 flex-shrink-0" />
                                <span class="text-amber-100">Net profit: KES {{ (bonusInfo.bonus_amount - bonusInfo.claim_cost).toLocaleString() }} added instantly</span>
                            </li>
                        </ul>
                    </div>

                    <!-- Terms & Conditions -->
                    <div class="bg-gradient-to-br from-orange-600/40 to-red-600/40 backdrop-blur-xl border border-orange-500/30 rounded-2xl p-6 shadow-2xl shadow-orange-500/20">
                        <h3 class="text-lg font-bold text-white mb-4 flex items-center">
                        ...
                        <ul class="space-y-2 text-sm">
                            <li class="flex items-start space-x-2">
                                <Info :size="16" class="text-orange-400 mt-0.5 flex-shrink-0" />
                                <span class="text-orange-100">One-time claim per bonus type</span>
                            </li>
                            <li class="flex items-start space-x-2">
                                <Info :size="16" class="text-orange-400 mt-0.5 flex-shrink-0" />
                                <span class="text-orange-100">Amount is deducted from deposit_balance</span>
                            </li>
                            <li class="flex items-start space-x-2">
                                <Info :size="16" class="text-orange-400 mt-0.5 flex-shrink-0" />
                                <span class="text-orange-100">Highest return bonus available</span>
                            </li>
                        </ul>
                    </div>
                </div>
...
import { 
    Crown, Loader2, CheckCircle, AlertCircle, 
    HelpCircle, FileText, Info, Gift, Star, Award
...
const claimBonus = async () => {
    ...
    try {
        await router.post(route('reward-center.stats'), {}, {
            preserveScroll: true,
            onSuccess: (page) => {
                // SAFELY check for flash messages
                const flash = page?.props?.flash || {};
                
                if (flash.success) {
                    alert(flash.success);
                    // Reload to update balance and claimed status
                    setTimeout(() => {
                        router.reload({ only: ['bonusInfo', 'hasClaimed', 'userBalance'] });
                    }, 1000);
                } else if (flash.error) {
                    alert(flash.error);
                }
